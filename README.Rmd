...
{parcel} requires the `usaddress` python module for tagging addresses and creating address stubs, as well as the `dedupe` and `dedupe-variable-address` python modules for matching addresses, all of which can be installed to an existing python environment from inside R using [reticulate](https://rstudio.github.io/reticulate/index.html):
